...
/// <summary>
/// 将UTF8编码的string转为GBK的string
/// </summary>
/// <param name="str">utf8编码的string</param>
/// <returns></returns>
...
/// <summary>
/// 将GBK编码的string转换为UTF-8
/// </summary>
/// <param name="str">GBK编码</param>
/// <returns></returns>
...
inline const void CHTTPClient::AddQuery(const QueryMap& queryMap)
{
    m_strURL.append("?");
    for (QueryMap::const_iterator it = queryMap.cbegin();
        it != queryMap.cend();
        ++it)
    {
       
        const std::string encodeValue = GBKToUTF8(it->second);
        char* output = curl_easy_escape(m_pCurlSession, encodeValue.c_str(), encodeValue.size());
        if (output) {
            m_strURL.append(it->first + "=" + output+"&");
            curl_free(output);
        }
    }
